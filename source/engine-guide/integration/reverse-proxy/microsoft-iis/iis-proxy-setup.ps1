#Change policy temporary for one session
...
function Install-ISAPIRewriteWithLogging {
    ...
    $file = Join-Path $modulePath "ISAPI_Rewrite3_0112_Lite_x64.msi"
...
foreach ($mod in $modules) {
    ...
    if ($mod.Name -eq "ISAPI_Rewrite") {
        Install-ISAPIRewriteWithLogging -modulePath (Split-Path $filePath)
        continue
    }
    ...
    if ($installMode -eq "online") {
        ...
        try {
            Invoke-WebRequest -Uri $mod.Url -OutFile $filePath -ErrorAction Stop
        } catch {
            Write-Warning "Download failed for '$($mod.Name)': $_"
            continue
        }
    } else {
...
Write-Host "Enabling Application Request Routing..."
Set-WebConfigurationProperty -pspath $site -filter "system.webServer/proxy" -name "enabled" -value "True"

Write-Host "Setting 'Preserve Host Header' to True..."
Set-WebConfigurationProperty -pspath $site -filter "system.webServer/proxy" -name "preserveHostHeader" -value "True"
