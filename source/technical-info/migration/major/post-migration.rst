...
the new Axon Ivy Engine, here's how you make your projects compatible with the
new Axon Ivy Engine version. 

#. Download the latest Axon Ivy Designer which is available for the release you
   are migrating to.
...
#. Migrate the project according to the description in section
   :ref:`project-convert` of the Axon Ivy Designer Guide. Usually, this is
   achieved by invoking the project conversion action on each project. Some
   manual adaptations may be necessary.
#. Test the migrated project in the Axon Ivy Designer.
#. All migrated projects must be re-deployed to the new, upgraded Axon Ivy
   Engine version. 

   #. Not only the projects in the :code:`RELEASED` application version must be
      updated and re-deployed, but all in the :code:`DEPRECATED` application
      versions too.
   #. Other versions in the states :code:`PREPARED` or :code:`ARCHIVED` may be
      easier to delete, rather than updating and redeploying.
