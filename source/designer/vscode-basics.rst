...
Before diving into the Axon Ivy-specific functionality provided by the Designer extension, this chapter introduces fundamental VS Code concepts.

VS Code itself is very well documented. For an in-depth introduction, refer to the `VS Code Documentation <https://code.visualstudio.com/docs>`_

Refer to this part about the UI: `VS Code UI <https://code.visualstudio.com/docs/editing/getting-started/userinterface>`_
...
A rule of thumb is that everything you can click is most likely also a command you can execute.
...
As explained later, the Designer contributes its own commands to VS Code.
...
You can configure almost all UIs and behaviours in the VS Code settings. They are a list of key value pairs in ``settings.json`` files. As with commands, there are many built-in settings, but also extensions can supply their own settings that steer the behaviour of the extension.
The Axon Ivy Designer supplies its own small set of settings, which are explained in more depth in the :ref:`designer-config`.

You can open the Settings UI by pressing ``Ctrl+,`` or by opening the Command Palette (``Ctrl+Shift+P``) and executing the command ``Preferences: Open Settings (UI)``.

.. warning::
    VS Code settings come in a layered system: User and Workspace settings. User settings are applied globally for each new Workspace you open, while Workspace settings only apply to your current workspace. If both are set, Workspace settings win.
...
The reason for this is, that you don't want to have more extensions installed than neccessary, since they might interfere.
...
This profile has only installed the Axon Ivy Designer and all the required Java and Maven extensions which are installed automatically.
Note that we only configured the extensions, all the other things like Settings or Keyboard Shortcuts work identical to the default. This way you dont have to recreate all the settings for each project but only the extensions that change.

This way, when you want to work on an Axon Ivy project, you open that folder and first switch to your "Axon Ivy" profile.
...
Extensions can contribute custom tree views to show their own data or functions in the user interface.
...
in the currently open VS Code workspace.
...
The Axon Ivy extensions contributes its own status bar.
